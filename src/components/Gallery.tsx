...
interface GalleryProps {
    items: GalleryItem[];
    defaultLayout?: "grid" | "list" | "horizontal";
}

export function Gallery({ items, defaultLayout = "grid" }: GalleryProps) {
    const [layout, setLayout] = useState<"grid" | "list" | "horizontal">(defaultLayout);

    // If defaultLayout is provided, ensure we use it (handling hydration if needed in future)
    useEffect(() => {
    ...
    return (
        <div className="space-y-8">
            {/* View Toggle - Only show if not in horizontal mode (immersive) */}
            ...
            <motion.div
                ...
                className={cn(
                    ...
                    layout === "grid"
                        ...
                        : layout === "list"
                            ? "flex flex-col gap-0"
                            : "flex overflow-x-auto snap-x snap-mandatory gap-6 pb-12 w-[calc(100vw-3rem)] md:w-full no-scrollbar"
                )}
            >
                    ...
                    {items.map((item, index) => (
                        <motion.div
                            ...
                            className={cn(
                                ...
                                layout === "horizontal" && "snap-center flex-shrink-0 h-[60vh] md:h-[80vh] relative w-auto"
                            )}
                        >
                            ...
                            {item.href && item.title ? (
                                <ProjectCard
                                    ...
                                    index={index}
                                    className={layout === "horizontal" ? "h-full" : undefined}
                                />
                            ) : (
                                <div className={cn("relative overflow-hidden rounded-sm bg-stone-900", layout === "horizontal" ? "h-full w-auto" : "aspect-video")}>
                                    {layout === "horizontal" ? (
                                        <img
                                            src={item.src}
                                            alt={item.alt}
